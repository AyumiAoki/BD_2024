...
void salvarBlocoNoArquivo(const std::vector<TableRow> &bloco, int bucket, std::ofstream &arquivo) {
    if (!arquivo.is_open()) {
...
void analisarArquivo(const std::string &nomeArquivo) {
    ...
    while (std::getline(arquivo, linha)) {
        ...
        if (tokensVerificados.size() == 7) {
            ...
            if (tamListBlocks[bucket] + tamanhoDados >= MAX_BLOCK_SIZE - 4) {
                salvarBlocoNoArquivo(blocos[bucket], bucket, arquivoSaida);
                blocos[bucket].clear();
    ...
    for (int i = 0; i < NUM_BUCKETS; i++) {
        if (!blocos[i].empty()) {
            salvarBlocoNoArquivo(blocos[i], i, arquivoSaida);
        }
    }
