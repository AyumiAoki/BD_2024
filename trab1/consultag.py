...
dashboard_query = {
   ...
   'g': """
    ...
    SELECT 
        r.idUser,
        p.idgroup,
        COUNT(r.idUser) AS count_customer_reviews,
        ROW_NUMBER() OVER (PARTITION BY p.idgroup ORDER BY COUNT(r.idUser) DESC) AS rank
    FROM 
        public.produto p
    INNER JOIN 
        public.review r ON r.idProduct = p.id
    GROUP BY 
        r.idUser, p.idgroup
)
SELECT 
...
def consultaG():
    ...
    if resultadoG:
        # Formatar e exibir os resultados da consulta 'g'
...
def formatarResultadoG(resultado):
    """Formata e exibe os 10 clientes que mais fizeram comentários por grupo de produto."""

    # Preparar os dados para a tabela
    tabela_dados = []
    for row in resultado:
        customer_id = row[0]  # O ID do cliente
        group_name = row[1]  # O nome do grupo
        count_reviews = row[2]  # Quantidade de comentários do cliente

        # Adicionar os dados à tabela
        tabela_dados.append([customer_id, group_name, count_reviews])

    # Exibir a tabela formatada usando 'tabulate'
    print(tabulate(tabela_dados, headers=["Cliente ID", "Grupo de Produto", "Número de Comentários"], tablefmt="fancy_grid"))
    print("-" * 80)
